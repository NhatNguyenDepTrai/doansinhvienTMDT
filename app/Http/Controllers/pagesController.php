...
class pagesController extends Controller
{
    ...
    public function post_dangnhap(Request $request)
    {
        ...
        $check_admin = ['email' => $request->email, 'password' => $request->password, 'role_id' => 4];
    ...
    public function loaisanpham($id)
    {
        ...
        $loaisanpham = Sanpham::where('loaisanpham_id', $id)->paginate(100);
        return view('pages_client.loaisanpham', compact('loaisanpham', 'tenlsp'));
    }
    public function thuonghieu($tenth)
    {
        $tenth = Crypt::decryptString($tenth);
        $tenth = unserialize($tenth);

        $thuonghieu = Sanpham::where('hangsx', $tenth)->paginate(100);
    ...
    public function updateGioHang(Request $rq)
    {
        $rowId = $rq->rowId;
        $qnt = $rq->quantity;
        Cart::update($rowId, ['qty' => $qnt]);
    ...
    public function mua_ngay($idsp)
    {
        ...
        return redirect('index/gio-hang');
    }
    ...
    public function thongtindonhang()
    {
        ...
        if (Auth::check()) {
            ...
            if ($diemthuong->diem == 0) {
                $tienquydoi = 0;
            } elseif (0 < $diemthuong->diem && $diemthuong->diem < 30) {
                $tienquydoi = 20000;
            } elseif (30 < $diemthuong->diem && $diemthuong->diem < 60) {
                $tienquydoi = 30000;
            } elseif (60 < $diemthuong->diem && $diemthuong->diem < 100) {
                $tienquydoi = 50000;
            } else {
                $tienquydoi = 70000;
            }
    ...
    public function postDonhang(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            if ($diemthuong->diem == 0) {
                $tienquydoi = 0;
            } elseif (0 < $diemthuong->diem && $diemthuong->diem < 10) {
                $tienquydoi = 10000;
            } elseif (10 < $diemthuong->diem && $diemthuong->diem < 20) {
                $tienquydoi = 20000;
            } elseif (20 < $diemthuong->diem && $diemthuong->diem < 40) {
                $tienquydoi = 30000;
            } elseif (40 < $diemthuong->diem && $diemthuong->diem < 70) {
                $tienquydoi = 50000;
            } elseif (70 < $diemthuong->diem && $diemthuong->diem < 100) {
                $tienquydoi = 70000;
            } else {
                $tienquydoi = 100000;
            }
        ...
        if ($request->tichdiem == 1) {
            ...
            $update_point = Tichdiem::where('user_id', Auth::User()->id)->update(['diem' => 0]);
        }
        ...
        if (100000 < $total_money && $total_money < 200000) {
            $prize_point = 5;
        } elseif (200000 < $total_money && $total_money < 300000) {
            $prize_point = 10;
        } elseif (300000 < $total_money && $total_money < 500000) {
            $prize_point = 15;
        } elseif (500000 < $total_money && $total_money < 700000) {
            $prize_point = 20;
        } elseif (700000 < $total_money && $total_money < 1000000) {
            $prize_point = 30;
        } elseif (1000000 < $total_money && $total_money < 2000000) {
            $prize_point = 40;
        } else {
            $prize_point = 50;
        }
        ...
        if ($donhang->save() && Auth::check()) {
            ...
            $update_point = Tichdiem::where('user_id', Auth::User()->id)->update(['diem' => $diemthem]);
        }
        ...
        foreach ($content as $product) {
            if (!isset($grouped[$product->options->mashop])) {
                $grouped[$product->options->mashop] = [];
            }
        ...
        foreach ($grouped as $shop_id => $items) {
            ...
            foreach ($items as $item) {
                ...
                $update_slx = Sanphamshop::where('sanpham_id', $item->id)->update(['soluongxuat' => $soluongxuat]);
            }
        }
